...
{
public:
	shared_ptr<TTAggregator> agg = NULL;
	LocallyAvailableOperatorContext<T>* pCtx = NULL;
	virtual void Initialize(OperatorContext* context) override
	{
		CHECK(context->typeCode == OperatorContext::LocallyAvailable);
		agg = make_shared<TTAggregator>();
		pCtx = (LocallyAvailableOperatorContext<T>*)context;
		CHECK(pCtx->inputAddrs.size() == 2);
		CHECK(pCtx->outputAddrs.size() == 1);
		CHECK(pCtx->lens)
	}

	virtual void Run() override
	{
		agg->VectorVectorAdd(pCtx->inputAddrs[0], pCtx->lens, pCtx->inputAddrs[1]);
	}
};