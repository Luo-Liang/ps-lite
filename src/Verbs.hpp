  ...
  public:
	...
	void initialize_device(const std::string desired_device_name, const int8_t desired_port)
	{
		...
		if (SocketCount < 0)
			...
			SocketCount = numa_num_task_nodes();
		...
		for (size_t i = 0; i < num_devices; ++i)
		{
			...
			for (int p = 1; p <= dev_attr.phys_port_cnt; p++)
			{
				...
				if ((desired_device_name.length() == 0 ||
					 (desired_device_name == ibv_get_device_name(name) && p == desired_port)) &&
					socket2CoreCnt.at(socketId) > 0 && blockedIFSet.find(nameStr) == blockedIFSet.end())
				{
      ...
      printf("[%d][%d] view on key from server/worker %d is : %d, key count = %d\n", MyID, myRole, i, serverQPCounts[i], serverKeyCounts[i]);
		...
		if (myRole == ps::Node::Role::SERVER)
		{
			...
			else
			{
				...
				{
					if (socket2CoreIdx.at(i).size() == 0)
						continue; //this socket does not have any core.
	...
	Verbs(int qpNum,
		...
		: devices(nullptr), num_devices(0), sendCompletionQueues(), receiveCompletionQueues(), endpoints(), workerQPCount(qpNum), DirectConnect(directConnect), myId(psliteid)
	{
