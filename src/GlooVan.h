...
namespace ps {
    ...
    */
	...
	virtual string VanType() override
	{
	    ...
	    auto pContext = std::make_shared<gloo::rendezvous::Context>(Postoffice::Get()->my_rank(), Postoffice::Get()->num_workers());
	    //std::stringstream ss;
	    //ss << "/uncached/Gloo/";
	    //if(ps::Postoffice::Get()->my_rank() == 0)
	    //{
	    ...
	    {
		Reducer = new gloo::AllreduceHalvingDoubling<float>(pContext, layerAddr, GlooBuffers.size());
	    }
	    else if(reducerString == "ring_chunked")
	    {
		Reducer = new gloo::AllreduceRingChunked<float>(pContext, layerAddr, GlooBuffers.size());
	    }
